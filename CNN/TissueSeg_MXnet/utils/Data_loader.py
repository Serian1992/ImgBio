...
class dataLoader(mx.io.DataIter):
    ...
    def __init__(self, data_list, input_args):
        ...
        self.data = dataLoader.read_data(self.data_list)
    ...
    def _worker(worker_id, data_queue, result_queue, input_args, stop_word, stop_flag):
        ...
        for item in iter(data_queue.get, stop_word):
            if stop_flag == 1:
                break
            image, label = dataLoader._get_single(item, input_args)
    ...
    def _get_next(self):
        ...
        for i in range(self.current, self.current + batch_size):
            if self.multi_thread:
                ...
                image, label = dataLoader._get_single(self.data[i], self.input_args)
    ...
    def _get_single(item, input_args):
        return utils.get_single_image(item, input_args)
